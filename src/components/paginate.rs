use poise::serenity_prelude::{InteractionResponseType, EmbedField};
use serenity::{self, collector::CollectComponentInteraction, builder::CreateEmbed};
use crate::{Context, commands::profile::ProfileInfo};
...
pub async fn paginate(
    ...
    ctx.send(|b| {
        b.embed(|b| b.description(pages.get(current_page).expect("No pages left to paginate!")))
            .components(|b| {
        ...
        press
            .create_interaction_response(ctx, |b| {
                b.kind(InteractionResponseType::UpdateMessage)
                    .interaction_response_data(|b| b.embed(|b| b.description(pages.get(current_page).expect("No pages left to paginate!"))))
            })
            .await?;
    }
